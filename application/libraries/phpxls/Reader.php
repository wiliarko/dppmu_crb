...
class OLERead {
	...
	function read($sFileName)
	{
		...
		if ($this->numExtensionBlocks != 0) $bbdBlocks = (BIG_BLOCK_SIZE - BIG_BLOCK_DEPOT_BLOCKS_POS) / 4;
		...
		for ($j = 0; $j < $this->numExtensionBlocks; $j++)
		{
			...
			if ($bbdBlocks < $this->numBigBlockDepotBlocks) $this->extensionBlock = GetInt4d($this->data, $pos);
		}
	...
	function __readPropertySets()
	{
		...
		while ($offset < strlen($this->entry))
		{
			...
			if ($name == "Root Entry") $this->rootentry = count($this->props) - 1;
	...
	function getWorkBook()
	{
		...
		else
		{
			...
			if ($this->props[$this->wrkbook]['size'] % BIG_BLOCK_SIZE != 0) $numBlocks++;
...
class Spreadsheet_Excel_Reader {
	...
	function getCol($col)
	{
		if (is_string($col))
		{
			...
			if (array_key_exists($col, $this->colnames)) $col = $this->colnames[$col];
		}
	...
	function style($row, $col, $sheet = 0, $properties = '')
	{
		...
		if ($height!= "") $css .= "font-size:$height"."px;";
		if ($bgcolor != "")
		{
	...
	function xfRecord($row, $col, $sheet = 0)
	{
		...
		if ($xfIndex != "") return $this->xfRecords[$xfIndex];
	...
	function xfProperty($row, $col, $sheet, $prop)
	{
		...
		if ($xfRecord != NULL) return $xfRecord[$prop];
	...
	function fontRecord($row, $col, $sheet = 0)
	{
		...
		if ($xfRecord != NULL)
		{
			...
			if ($font != NULL) return $this->fontRecords[$font];
		}
    ...
    {
		if (($ci <> 0x7FFF) && ($ci <> '')) return $this->colors[$ci];
		...
		for ($row = 1; $row <= $this->rowcount($sheet); $row++)
		{
			...
			for ($col = 1; $col <= $this->colcount($sheet); $col++)
			{
				...
				for ($i = 0; $i < $rowspan; $i++)
				{
					for($j = 0; $j < $colspan; $j++)
					{
						if ($i > 0 OR $j > 0) $this->sheets[$sheet]['cellsInfo'][$row+$i][$col+$j]['dontprint'] = 1;
					}
				}
	...
	{
		if (( ! $f && $format == "%s") OR ($f == 49) OR ($format == "GENERAL"))
		...
		if ($has_commas) $pattern = preg_replace("/,/", "", $pattern);
		...
		if (preg_match($number_regex, $pattern, $matches))
		{
			...
			if ($has_commas) $formatted = number_format($num, strlen($right));
			else
			{
	...
	function __construct($file = '', $store_extended_info = TRUE, $outputEncoding = '')
	{
		...
		if ($outputEncoding != '') $this->setOutputEncoding($outputEncoding);
		for ($i = 1; $i < 245; $i++)
		{
	...
	function setUTFEncoder($encoder = 'iconv')
	{
		...
		if ($encoder == 'iconv') $this->_encoderFunction = function_exists('iconv') ? 'iconv' : '';
		elseif ($encoder == 'mb') $this->_encoderFunction = function_exists('mb_convert_encoding') ? 'mb_convert_encoding' : '';
	}
	...
	function read($sFileName)
	{
		...
		if ($res  === FALSE)
		{
			if($this->_ole->error == 1) die('The filename '.$sFileName.' is not readable');
		}
			...
			switch ($code)
			{
				...
				case SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
					...
					if ($version == SPREADSHEET_EXCEL_READER_BIFF8)
					{
						...
						if (ord($data[$pos + 8]) == 0) $formatString = substr($data, $pos + 9, $numchars);
						else $formatString = substr($data, $pos + 9, $numchars * 2);
					}
					else
					{
				...
				case SPREADSHEET_EXCEL_READER_TYPE_FONT:
					...
					$font		= "";
					$numchars	= ord($data[$pos + 
						18]);
					if ((ord($data[$pos + 19]) & 1) == 0) $font = substr($data, $pos + 20, $numchars);
					else
					{
				...
				case SPREADSHEET_EXCEL_READER_TYPE_XF:
					...
					$xf['borderLeft']		= isset($this->lineStyles[($border & 0xF)]) ? $this->lineStyles[($border & 0xF)] : '';
					$xf['borderRight']		= isset($this->lineStyles[($border & 0xF0) >> 4]) ? $this->lineStyles[($border & 0xF0) >> 4] : '';
					$xf['borderTop']		= isset($this->lineStyles[($border & 0xF00) >> 8]) ? $this->lineStyles[($border & 0xF00) >> 8] : '';
					$xf['borderBottom']		= isset($this->lineStyles[($border & 0xF000) >> 12]) ? $this->lineStyles[($border & 0xF000) >> 12] : '';
				...
				case SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
					...
					if ($version == SPREADSHEET_EXCEL_READER_BIFF8)
					{
						...
						if ($chartype == 0) $rec_name = substr($data, $pos + 12, $rec_length);
						else $rec_name = $this->_encodeUTF16(substr($data, $pos + 12, $rec_length * 2));
					}
					elseif ($version == SPREADSHEET_EXCEL_READER_BIFF7) $rec_name = substr($data, $pos + 11, $rec_length);
	...
	function _parsesheet($spos)
	{
		...
		if ($substreamType != SPREADSHEET_EXCEL_READER_WORKSHEET) return -2;
		...
		while($cont)
		{
			...
			switch ($code)
			{
				case SPREADSHEET_EXCEL_READER_TYPE_DIMENSION:
					...
					{
						if (($length == 10) OR ($version == SPREADSHEET_EXCEL_READER_BIFF7))
						{
				...
				case SPREADSHEET_EXCEL_READER_TYPE_MERGEDCELLS:
					...
					for ($i = 0; $i < $cellRanges; $i++)
					{
						...
						if ($lr - $fr > 0) $this->sheets[$this->sn]['cellsInfo'][$fr + 1][$fc + 1]['rowspan'] = $lr - $fr + 1;
						if ($lc - $fc > 0) $this->sheets[$this->sn]['cellsInfo'][$fr + 1][$fc + 1]['colspan'] = $lc - $fc + 1;
					}
				break;
				...
				case SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
					...
					if ($this->isDate($spos)) $numValue = $tmp['double'];
					else $numValue = $this->createNumber($spos);
				...
				case SPREADSHEET_EXCEL_READER_TYPE_FORMULA2:
					...
					elseif ((ord($data[$spos + 6]) == 1)
					&& (ord($data[$spos + 12]) == 255)
					&& (ord($data[$spos + 13]) == 255))
					{
                        if (ord($this->data[$spos + 8]) == 1) $this->addcell($row, $column, "TRUE");
                        else $this->addcell($row, $column, "FALSE");
					}
					elseif ((ord($data[$spos + 6]) == 2)
					&& (ord($data[$spos + 12]) == 255)
					&& (ord($data[$spos + 13]) == 255))
					{}
					elseif ((ord($data[$spos + 6]) == 3)
					&& (ord($data[$spos + 12]) == 255)
					&& (ord($data[$spos + 13]) == 255))
				...
				case SPREADSHEET_EXCEL_READER_TYPE_ROW:
					...
					if (($rowInfo & 0x8000) > 0) $rowHeight = -1;
					else $rowHeight = $rowInfo & 0x7FFF;
	...
	function _getCellDetails($spos, $numValue, $column)
	{
		...
		if (isset($this->_columnsFormat[$column + 1])) $format = $this->_columnsFormat[$column + 1];
