...
function gmgetdate($ts = null){
	...
	return(array_comb($k,explode(":",gmdate('s:i:G:j:w:n:Y:z:l:F:U',is_null($ts)?time():$ts)))); //return(array_comb($k,split(":",gmdate('s:i:G:j:w:n:Y:z:l:F:U',is_null($ts)?time():$ts))));
	} 
...
class Spreadsheet_Excel_Reader {
	...
	function rowheight($row,$sheet=0) {
		return isset($this->rowInfo[$sheet][$row]['height']) ? $this->rowInfo[$sheet][$row]['height'] : '';
	}
	...
	// =========================================
	 function getSheetIndex($label) {
		foreach ($this->boundsheets as $key => $item)
			if (strtolower($item['name']) == strtolower($label))
				return $key;
	...
	function dump_specified($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel',$startrow=1,$endrow=0,$startcol=1,$endcol=0) {
...
colspan=$colspan":"") . ($rowspan > 1?" rowspan=$rowspan":"") . ">";
	...
	function _format_value($format,$num,$f) {
		...
		$parts = explode(";",$format); //split(";",$format);
		$pattern = $parts[0];
		// Negative pattern
